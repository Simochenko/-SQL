...
select title, sum(ba) as Количество,SUM(ba*price) as Сумма from (select title, sum(buy_book.amount) as ba, price
from book
 JOIN buy_book USING(book_id)
 JOIN buy USING(buy_id)
 JOIN buy_step USING(buy_id)
WHERE  date_step_end IS NOT Null and YEAR(date_step_end) = 2020 and step_id = 1
...
2 Создать другой запрос, который выбирает название Книги, ее количество и стоимость для предыдущего года. Для этого использовать таблицы buy_archive и book. Количество и цену взять из таблицы buy_archive. Отладить этот запрос

3. Соединить эти запросы с помощью UNION ALL. Тоже запустить и посмотреть результат

4. Оформить полученный запрос как вложенный, разместить его во FROM , в SELECT вывести Книги, просуммировать количество и стоимость из вложенного запроса, выполнить группировку по названию Книги. Ну и сортировку по условию
